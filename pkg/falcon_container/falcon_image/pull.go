...
func Pull(apiCfg *falcon.ApiConfig, logger logr.Logger) (*FalconImage, error) {
	fi := FalconImage{
		tar: bytes.Buffer{},
	}
	return &fi, fi.pull(apiCfg, logger)
}

func (fi *FalconImage) pull(apiCfg *falcon.ApiConfig, logger logr.Logger) error {
	client, err := falcon.NewClient(apiCfg)
	if err != nil {
		return err
	}
	filter := fmt.Sprintf("os:\"%s\"", "Container")
	logger.Info("Getting list of Falcon Container images")
	...
	for _, errorMsg := range payload.Errors {
		if errorMsg != nil && errorMsg.Message != nil {
			logger.Error(errors.New(*errorMsg.Message), "Received error from the Falcon server")
		}
	}
	...
	logger.Info("Downloaded Falcon Container Sensor", "falcon_sensor", *sensor.Name)
