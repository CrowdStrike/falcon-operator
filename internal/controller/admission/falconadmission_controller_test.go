...
var _ = Describe("FalconAdmission controller", func() {
	Context("FalconAdmission controller test", func() {
		...
		const (
			...
			ServiceAccountName           = "falcon-operator-admission-controller"
		)
		...
		It("should successfully reconcile a custom resource for FalconAdmission", func() {
			...
			falconAdmission.Spec.AdmissionConfig.AdmissionControlEnabled = &admissionControlEnabled
			...
			Eventually(func() error {
				...
				return k8sClient.Get(ctx, types.NamespacedName{Name: ServiceAccountName, Namespace: namespaceName}, found)
			}, time.Minute, time.Second).Should(Succeed())
			...
			Expect(configMap.Data["__CS_ADMISSION_CONTROL_ENABLED"]).To(Equal(fmt.Sprintf("%t", admissionControlEnabled)))
			...
			By("Deleting FalconAdmission")
			Eventually(func() error {
				return k8sClient.Delete(ctx, falconAdmission)
			})
			Expect(err).To(Not(HaveOccurred()))
		})
		...
		It("should successfully reconcile a custom resource for FalconAdmission", func() {
			...
			By("Checking if falcon-client container has required resources when admission controller is disabled")
			var falconClientContainer *corev1.Container
			for _, container := range deployment.Spec.Template.Spec.Containers {
				if container.Name == "falcon-client" {
					falconClientContainer = &container
					fmt.Print("Inside falcon client assignment")
					break
				}
			}
			fmt.Print("Inside falcon client assignment")
