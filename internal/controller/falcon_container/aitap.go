...
func (r *FalconContainerReconciler) reconcileAITapSecrets(ctx context.Context, log logr.Logger, falconContainer *falconv1alpha1.FalconContainer) (*corev1.SecretList, error) {
	...
	if falconContainer.Spec.Injector.AITap.AidrCollectorApiToken == "" {
		log.V(1).Info("AITap AI-DR token not configured, skipping AITap secret reconciliation")
		return secretList, nil
	}

	if falconContainer.Spec.Injector.AITap.UseExternalSecret {
		log.Info("AITap configured to use external secret, skipping secret creation")
		return secretList, nil
	}
...
func (r *FalconContainerReconciler) getAITapTargetNamespaces(ctx context.Context, log logr.Logger, falconContainer *falconv1alpha1.FalconContainer) ([]string, error) {
	...
	targetNamespaces = append(targetNamespaces, falconContainer.Spec.InstallNamespace)
	...
	if falconContainer.Spec.Injector.AITap.AllNamespaces {
		...
		for _, ns := range nsList.Items {
			...
			if ns.Name != falconContainer.Spec.InstallNamespace {
				targetNamespaces = append(targetNamespaces, ns.Name)
			}
		}
		log.Info("AITap AllNamespaces enabled, deploying to all namespaces", "count", len(targetNamespaces))
		return targetNamespaces, nil
	}

	if falconContainer.Spec.Injector.AITap.Namespaces != "" {
		namespaceList := strings.Split(falconContainer.Spec.Injector.AITap.Namespaces, ",")
		for _, ns := range namespaceList {
			ns = strings.TrimSpace(ns)
			if ns != "" && ns != falconContainer.Spec.InstallNamespace {
...
func (r *FalconContainerReconciler) getAITapSecretName(falconContainer *falconv1alpha1.FalconContainer) string {
	...
	return fmt.Sprintf("%s-%s", falconContainer.Name, common.FalconAITapAidrSecretName)
}
