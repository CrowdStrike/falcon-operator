...
# Install golangci-lint using go install with versioned binary
# Usage: install-golangci-lint.sh <target-path> <version>

set -e

if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <target-path> <version>" >&2
    echo "Example: $0 ./bin/golangci-lint v1.62.2" >&2
    exit 1
fi
...
# Install the tool
echo "Downloading github.com/golangci/golangci-lint/cmd/golangci-lint@${VERSION}"
mkdir -p "$LOCALBIN"
rm -f "$TARGET_PATH"

GOBIN="$LOCALBIN" go install "github.com/golangci/golangci-lint/cmd/golangci-lint@${VERSION}"
...
echo "Installed golangci-lint $VERSION to $TARGET_PATH"
