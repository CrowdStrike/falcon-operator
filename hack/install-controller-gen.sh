...
# Install controller-gen using go install with versioned binary
# Usage: install-controller-gen.sh <target-path> <version>

set -e

if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <target-path> <version>" >&2
    echo "Example: $0 ./bin/controller-gen v0.20.1" >&2
    exit 1
fi
...
# Install the tool
echo "Downloading sigs.k8s.io/controller-tools/cmd/controller-gen@${VERSION}"
mkdir -p "$LOCALBIN"
rm -f "$TARGET_PATH"

GOBIN="$LOCALBIN" go install "sigs.k8s.io/controller-tools/cmd/controller-gen@${VERSION}"
...
echo "Installed controller-gen $VERSION to $TARGET_PATH"
